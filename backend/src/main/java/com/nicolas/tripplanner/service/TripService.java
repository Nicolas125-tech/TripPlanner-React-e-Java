...
public class TripService {
    ...
    // Added `sync = true` to prevent Cache Stampedes (Thundering Herd) under high concurrency when a cache expires.
    ...
    @Caching(
        evict = { @CacheEvict(value = {CACHE_ALL_TRIPS, CACHE_SEARCH_TRIPS, CACHE_TRIPS_BY_CATEGORY}, allEntries = true) },
        put = { @CachePut(value = CACHE_TRIP, key = "#result.id") }
    )
    @Transactional
    ...
    @Caching(
        evict = { @CacheEvict(value = {CACHE_ALL_TRIPS, CACHE_SEARCH_TRIPS, CACHE_TRIPS_BY_CATEGORY}, allEntries = true) },
        put = { @CachePut(value = CACHE_TRIP, key = "#id") }
    )
    @Transactional
