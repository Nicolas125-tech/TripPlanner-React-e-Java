...
## 2024-05-18 - Optimized React State Favorites Lookup
**Learning:** While iterating through arrays via `.filter()` and `.includes()` is slow for O(N) operations on large datasets, simply replacing it with native array operations like `.indexOf()` and `.splice()` is merely a micro-optimization that doesn't change the time complexity class. To truly optimize frequent lookup and toggle operations, refactoring the data structure itself to a `Set` provides genuine O(1) performance, though it requires ensuring all dependent components and serialization methods are updated to handle the `Set` appropriately.
**Action:** When facing slow array lookups (`.includes()`), always evaluate if changing the underlying data structure to a `Set` or `Map` is feasible instead of just replacing the array iteration method. Remember to convert `Set`s to arrays before `JSON.stringify()`.
## 2026-09-02 - [Performance Optimization] Canceling in-flight API Requests\n**Learning:** When users trigger rapid, successive API calls (like typing in a search bar), older requests can resolve after newer ones, causing race conditions and stale state updates, while simultaneously wasting bandwidth and parsing resources.\n**Action:** Use an `AbortController` to actively cancel previous in-flight requests when a new request is initiated. When doing so, ensure that mock test assertions for `fetch` are updated to expect the second argument (`{ signal: ... }`) to avoid test failures.
## 2026-09-03 - [Performance Optimization] Canceling in-flight API Requests\n**Learning:** When users trigger rapid, successive API calls (like typing in a search bar), older requests can resolve after newer ones, causing race conditions and stale state updates, while simultaneously wasting bandwidth and parsing resources.\n**Action:** Use an `AbortController` to actively cancel previous in-flight requests when a new request is initiated. When doing so, ensure that mock test assertions for `fetch` are updated to expect the second argument (`{ signal: ... }`) to avoid test failures.
...
## 2026-09-10 - [Performance Optimization] Array vs Set for React State Iteration
**Learning:** For single-item insertions or deletions in React state arrays (like toggling a favorite), converting the array to a `Set` and back to leverage O(1) Set operations introduces more overhead than using native O(N) array operations (`indexOf()`, `splice()`, and `push()`), making it slower overall. Exposing a `Set` in context values when consumers expect an `Array` can cause crashes. However, for iterative operations like `filter()` over a large list, using a `Set` for O(1) lookups is vastly superior.
**Action:** Keep the base React state as an `Array` and use native operations (`indexOf` and `splice` on a clone) for single-item manipulations. Then, generate a `Set` using `useMemo` strictly for optimizing O(1) lookups during array iteration or rendering lists.
...
## 2024-09-14 - [Performance Optimization] Prevent DOM Destruction on Tab Switch
**Learning:** Using conditional rendering (e.g., `{activeTab === 'home' && <List />}`) for heavy tab contents forces React to destroy and recreate large DOM trees on every tab switch, defeating the reconciliation engine and causing severe layout thrashing.
**Action:** Instead of conditional unmounting, wrap tab contents in a div and use CSS-based visibility toggling (e.g., `className={activeTab === 'home' ? 'block' : 'hidden'}`). This retains existing DOM nodes in memory, enabling near-instantaneous tab switching with O(1) rendering cost.
