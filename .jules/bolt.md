...
## 2024-08-20 - Prevent UI Loading State Overflow
**Learning:** When using absolute positioned CSS loading overlays (e.g., `absolute inset-0 bg-white/50`) over lists or grids that dynamically populate (like `filteredDestinations`), the parent container must have a minimum height (e.g., `min-h-[300px]`). Otherwise, during the initial load when the array is empty, the container collapses to 0px height, causing the loading indicator text to overflow awkwardly onto adjacent elements.
**Action:** Always add a `min-h` utility class to the relative parent container when implementing absolute loading overlays to ensure they render correctly even when child content is absent.
